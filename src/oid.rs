...
impl Oid {
    ...
    /// View this OID as a byte-slice of the logical OID length.
    ///
    /// * 20 bytes for SHA1 OID
    /// * 32 bytes for SHA256 OID
    ///
    /// To access the full underlying buffer, use [`Oid::raw_bytes`].
    ...
    ///
    /// Without `unstable-sha256`, this is equivalent to [`Oid::as_bytes`].
    ///
    /// With `unstable-sha256`, this always returns the full 32-byte buffer,
    /// which may include zero-padding for SHA1 OIDs.
...
mod tests {
    ...
    fn object_format_from_oid() {
        ...
        let sha1_from_str = Oid::from_str(
            "decbf2be529ab6557d5429922251e5ee36519817",
            ObjectFormat::Sha1,
        )
        .unwrap();
        ...
        let sha256_from_str = Oid::from_str(
            "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef",
            ObjectFormat::Sha256,
        )
        .unwrap();
